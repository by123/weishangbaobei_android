    ...
    public void onMoving(boolean z, float f, int i, int i2, int i3) {
        ...
        if (z || (!this.mProgress.isRunning() && !this.mFinished)) {
            if (this.mState != RefreshState.Refreshing) {
                ...
                this.mProgress.showArrow(true);
                this.mProgress.setStartEndTrim(CropImageView.DEFAULT_ASPECT_RATIO, Math.min(MAX_PROGRESS_ANGLE, f3));
                this.mProgress.setArrowScale(Math.min(1.0f, max));
                this.mProgress.setProgressRotation((((max * 0.4f) - 16.0f) + (((float) (max2 - pow)) * 2.0f * 2.0f)) * 0.5f);
            }
            ImageView imageView = this.mCircleView;
            float f4 = (float) i;
            imageView.setTranslationY(Math.min(f4, (float) ((i / 2) + (this.mCircleDiameter / 2))));
            imageView.setAlpha(Math.min(1.0f, (f4 * 4.0f) / ((float) this.mCircleDiameter)));
        }
    }
