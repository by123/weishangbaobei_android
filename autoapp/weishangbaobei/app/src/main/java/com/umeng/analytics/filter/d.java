...
public class d {
        ...
        if (z) {
            try {
                ...
                while (i < decode.length / 4) {
                    int i2 = i * 4;
                    this.d.add(Integer.valueOf(((decode[i2 + 0] & 255) << 24) + ((decode[i2 + 1] & 255) << ap.n) + ((decode[i2 + 2] & 255) << 8) + (decode[i2 + 3] & 255)));
                    i++;
                }
    ...
    private Integer c(String str) {
        try {
            ...
            return Integer.valueOf(((digest[0] & 255) << 24) + ((digest[1] & 255) << ap.n) + ((digest[2] & 255) << 8) + (digest[3] & 255));
        } catch (Exception e2) {
            e2.printStackTrace();
            return null;
        }
