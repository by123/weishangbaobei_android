...
public class VoiceRecordAdapter extends BaseAdapter {
    ...
    public VoiceRecordAdapter(Context context, List<RecordBean> list) {
    ...
    public View getView(final int i, View view, ViewGroup viewGroup) {
        ...
        viewHolder.playLayout.setOnClickListener(new View.OnClickListener() {
            public void onClick(View view) {
                if (!VoiceRecordAdapter.this.isHasPlay) {
                    boolean unused = VoiceRecordAdapter.this.isHasPlay = true;
                    VoiceRecordAdapter.this.resetData(i);
                ...
                boolean unused2 = VoiceRecordAdapter.this.isHasPlay = false;
                AnimationDrawable animationDrawable2 = (AnimationDrawable) linearLayout.getBackground();
                VoiceRecordAdapter.this.resetAnim(animationDrawable2);
                recordBean.setIsPlaying(false);
                recordBean.setIsPlayed(true);
                SQLiteUtils.getInstance().updateRecord(recordBean);
                MediaManager.release();
            ...
            public void onClick(View view) {
                DialogUIUtils.dialogDefault(AppManager.getAppManager().currentActivity(), "", "是否删除本条语音", "取消", "确定", (View.OnClickListener) null, new View.OnClickListener() {
                    public void onClick(View view) {
                        ...
                        VoiceRecordAdapter.this.mRecords.remove(recordBean);
                        VoiceRecordAdapter.this.notifyDataSetChanged();
                    }
                });
            }
