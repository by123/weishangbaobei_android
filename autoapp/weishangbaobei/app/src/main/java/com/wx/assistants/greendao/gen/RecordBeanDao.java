...
public class RecordBeanDao extends AbstractDao<RecordBean, Long> {
    ...
    public RecordBean readEntity(Cursor cursor, int i) {
        ...
        int i7 = cursor2.getInt(i + 5);
        boolean z = cursor2.getShort(i + 6) != 0;
        boolean z2 = cursor2.getShort(i + 7) != 0;
        boolean z3 = cursor2.getShort(i + 8) != 0;
        long j = cursor2.getLong(i + 9);
        int i8 = i + 10;
        if (!cursor2.isNull(i8)) {
            str = cursor2.getString(i8);
        }
        return new RecordBean(valueOf, string, string2, string3, string4, i7, z, z2, z3, j, str, cursor2.getInt(i + 11), cursor2.getShort(i + 12) != 0);
    }
