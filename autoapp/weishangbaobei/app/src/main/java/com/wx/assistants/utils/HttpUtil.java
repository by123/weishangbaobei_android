...
public class HttpUtil {
    ...
    private Retrofit retrofit = new Retrofit.Builder().baseUrl(QBangApis.DEV_BASE_URL).addConverterFactory(MyConverterFactory.create()).client(new OkHttpClient.Builder().addInterceptor(new Interceptor() {
        public Response intercept(Interceptor$Chain interceptor$Chain) throws IOException {
            ...
            return interceptor$Chain.proceed(request.newBuilder().header("equipment", MacAddressUtils.getMacAddress(MyApplication.mContext)).header("platform", "android").header("version_code", String.valueOf(350)).header("version_name", "3.5.0.10").header(e.n, MyApplication.WS).method(request.method(), request.body()).build());
        }
    ...
    public Request addBasicParams(Request request) {
        ...
        hashMap.put("version_name", "3.5.0.10");
