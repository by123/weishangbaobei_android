...
public class LoadCircleView extends View {
    ...
    public void onDraw(Canvas canvas) {
        int i = this.mWidth >> 1;
        int i2 = (this.mWidth >> 1) - 8;
        if (this.currentLineIndex >= 12) {
            this.currentLineIndex = 0;
        }
        for (int i3 = 0; i3 < 12; i3++) {
            if (i3 < this.currentLineIndex + 4 && i3 >= this.currentLineIndex) {
                this.mPaint.setColor(-7829368);
            } else if (this.currentLineIndex <= 8 || i3 >= (this.currentLineIndex + 4) - 12) {
            ...
            Double.isNaN(d2);
            Double.isNaN(d);
            canvas.drawLine(f, (float) (d + (d2 * 0.5d)), f, (float) (i2 * 2), this.mPaint);
