...
public class SignSeekBar extends View {
    ...
    public SignSeekBar(Context context, AttributeSet attributeSet, int i) {
        ...
        this.isSidesLabels = (this.mSidesLabels == null || this.mSidesLabels.length <= 0) ? false : z;
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (this.isShowSignBorder) {
            paddingTop += (float) this.mSignBorderSize;
        }
        if (this.isShowSign && !this.isSignArrowAutofloat) {
            paddingLeft += (float) ((this.mSignWidth / 2) + this.mSignBorderSize);
        ...
        if (this.isShowSectionText) {
            ...
            if (this.mSectionTextPosition == 0) {
                ...
                this.mPaint.getTextBounds(minText, 0, minText.length(), this.mRectText);
                canvas.drawText(minText, (((float) this.mRectText.width()) / 2.0f) + paddingLeft, height, this.mPaint);
                ...
                float f = ((float) this.mThumbRadiusOnDragging) + paddingTop + ((float) this.mTextSpace);
                String minText2 = this.isSidesLabels ? this.mSidesLabels[0] : getMinText();
                this.mPaint.getTextBounds(minText2, 0, minText2.length(), this.mRectText);
                float height2 = f + ((float) this.mRectText.height());
                float f2 = this.mLeft;
                if (this.mSectionTextPosition == 1) {
        ...
        if (z || this.isShowSectionMark) {
            drawMark(canvas, paddingLeft, paddingTop, z, true);
        }
        ...
        float f4 = paddingTop;
        float f5 = paddingTop;
        canvas.drawLine(paddingLeft, f4, this.mThumbCenterX, f5, this.mPaint);
        this.mPaint.setColor(this.mTrackColor);
        this.mPaint.setStrokeWidth((float) this.mTrackSize);
        canvas.drawLine(this.mThumbCenterX, f4, measuredWidth, f5, this.mPaint);
        this.mPaint.setColor(this.mThumbColor);
        if (this.isShowThumbShadow) {
            canvas.drawCircle(this.mThumbCenterX, paddingTop, ((float) (this.isThumbOnDragging ? this.mThumbRadiusOnDragging : this.mThumbRadius)) * this.mThumbRatio, this.mPaint);
    ...
    private void drawMark(Canvas canvas, float f, float f2, boolean z, boolean z2) {
        ...
        float height = ((float) this.mRectText.height()) + f2 + ((float) this.mThumbRadiusOnDragging) + ((float) this.mTextSpace);
        for (int i = 0; i <= this.mSectionCount; i++) {
            float f3 = (float) i;
            float f4 = (this.mSectionOffset * f3) + f;
            ...
            if (z) {
                ...
                this.mPaint.setColor((!isEnabled() && Math.abs(this.mProgress - f5) > CropImageView.DEFAULT_ASPECT_RATIO) ? this.mUnusableColor : this.mSectionTextColor);
                if (this.mSectionTextInterval > 1) {
                    if (z2 && i % this.mSectionTextInterval == 0) {
                        if (this.isSidesLabels) {
                            ...
                            if (this.isFloatType) {
                                str2 = float2String(f5);
                            } else {
                                str2 = ((int) f5) + "";
                            }
                            canvas.drawText(str2, f4, height, this.mPaint);
                        }
                    }
                    ...
                    if (!this.isSidesLabels || i / this.mSectionTextInterval > this.mSidesLabels.length) {
                        ...
                        canvas.drawText(str, f4, height, this.mPaint);
                    } else {
                        canvas.drawText(this.mSidesLabels[i / this.mSectionTextInterval], f4, height, this.mPaint);
                    }
                }
            }
        }
    ...
    private void drawThumbText(Canvas canvas, float f) {
        ...
        this.mPaint.setTextSize((float) this.mThumbTextSize);
        this.mPaint.getTextBounds("0123456789", 0, "0123456789".length(), this.mRectText);
        float height = f + ((float) this.mRectText.height()) + ((float) this.mThumbRadiusOnDragging) + ((float) this.mTextSpace);
    ...
    private void drawValueSign(Canvas canvas, int i, int i2) {
        ...
        if (this.valueSignBounds.left < getPaddingLeft()) {
            ...
            this.roundRectangleBounds.set((float) (this.valueSignBounds.left + paddingLeft), (float) this.valueSignBounds.top, (float) (this.valueSignBounds.right + paddingLeft), (float) this.valueSignBounds.bottom);
    ...
    private void createValueTextLayout() {
        ...
        if (this.isShowProgressInFloat) {
            ...
            if (this.mFormat != null) {
                str = this.mFormat.format((double) progressFloat);
            }
        } else {
            int progress = getProgress();
            str = String.valueOf(progress);
            if (this.mFormat != null) {
                str = this.mFormat.format((long) progress);
            }
        }
        if (this.mValueFormatListener != null) {
            str = this.mValueFormatListener.format(Float.parseFloat(str));
        } else if (!(str == null || this.unit == null || this.unit.isEmpty())) {
            if (!this.mReverse) {
                str = str + String.format(" <small>%s</small> ", new Object[]{this.unit});
            } else {
                str = String.format(" %s ", new Object[]{this.unit}) + str;
            }
        }
        this.valueTextLayout = new StaticLayout(Html.fromHtml(str), this.valueTextPaint, this.mSignWidth, Layout.Alignment.ALIGN_CENTER, 1.0f, CropImageView.DEFAULT_ASPECT_RATIO, false);
    }
    ...
    private void drawProgressText(Canvas canvas) {
        ...
        drawCircleText(canvas, paint, this.mThumbCenterX, (float) (getPaddingTop() + this.mThumbRadiusOnDragging), f, str);
    }
    ...
    private void drawCircleText(Canvas canvas, Paint paint, float f, float f2, float f3, String str) {
        ...
        canvas.drawText(str, ((f - f3) + f3) - ((float) (rect.width() / 2)), ((f2 - f3) + ((((f3 * 2.0f) - ((float) fontMetricsInt.bottom)) + ((float) fontMetricsInt.top)) / 2.0f)) - ((float) fontMetricsInt.top), paint);
    }
    ...
    private boolean isThumbTouched(MotionEvent motionEvent) {
        ...
        if (((motionEvent.getX() - f2) * (motionEvent.getX() - f2)) + ((motionEvent.getY() - measuredHeight) * (motionEvent.getY() - measuredHeight)) <= (this.mLeft + f) * (this.mLeft + f)) {
            return true;
        }
        return false;
    }
    ...
    public void autoAdjustSection() {
        ...
        if (!z) {
            if (this.mThumbCenterX - f <= this.mSectionOffset / 2.0f) {
                ofFloat = ValueAnimator.ofFloat(new float[]{this.mThumbCenterX, f});
            } else {
