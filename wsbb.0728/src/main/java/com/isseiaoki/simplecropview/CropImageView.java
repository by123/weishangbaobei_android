...
public class CropImageView extends ImageView {
    ...
    public static class SavedState extends View.BaseSavedState {
        ...
        private SavedState(Parcel parcel) {
            ...
            this.isCropEnabled = parcel.readInt() == 0 ? false : z;
    ...
    private void adjustRatio() {
        if (this.mImageRect != null) {
            ...
            this.mFrameRect = new RectF(f14 - f16, f15 - f17, f16 + f14, f17 + f15);
            invalidate();
        }
    }
    ...
    private void drawEditFrame(Canvas canvas) {
        if (this.mIsCropEnabled) {
            ...
            if (this.mShowGuide) {
                ...
                float f = ((this.mFrameRect.right - this.mFrameRect.left) / 3.0f) + this.mFrameRect.left;
    ...
    private void initLayout(int i, int i2) {
        ...
        setCenter(new PointF((f * 0.5f) + ((float) getPaddingLeft()), (f2 * 0.5f) + ((float) getPaddingTop())));
        setScale(f5);
        initCropFrame();
        adjustRatio();
    ...
    private void moveHandleLB(float f, float f2) {
        if (this.mCropMode == CropMode.RATIO_FREE) {
            ...
            this.mFrameRect.bottom += f2;
            if (isWidthTooSmall()) {
                float f3 = this.mMinFrameSize;
                float frameW = getFrameW();
                this.mFrameRect.left -= f3 - frameW;
            }
            if (isHeightTooSmall()) {
                float f4 = this.mMinFrameSize;
                float frameH = getFrameH();
                RectF rectF = this.mFrameRect;
                rectF.bottom = (f4 - frameH) + rectF.bottom;
            }
            checkScaleBounds();
            return;
        }
        ...
        if (!isInsideHorizontal(this.mFrameRect.left)) {
            float f5 = this.mImageRect.left - this.mFrameRect.left;
            this.mFrameRect.left += f5;
            this.mFrameRect.bottom -= (f5 * getRatioY()) / getRatioX();
        }
        if (!isInsideVertical(this.mFrameRect.bottom)) {
            float f6 = this.mFrameRect.bottom - this.mImageRect.bottom;
            this.mFrameRect.bottom -= f6;
            float ratioX = (f6 * getRatioX()) / getRatioY();
    ...
    private void moveHandleLT(float f, float f2) {
        ...
        if (isWidthTooSmall()) {
            float frameW2 = this.mMinFrameSize - getFrameW();
            this.mFrameRect.left -= frameW2;
            this.mFrameRect.top -= (frameW2 * getRatioY()) / getRatioX();
        }
        if (isHeightTooSmall()) {
            float frameH2 = this.mMinFrameSize - getFrameH();
        ...
        if (!isInsideHorizontal(this.mFrameRect.left)) {
            float f5 = this.mImageRect.left - this.mFrameRect.left;
            this.mFrameRect.left += f5;
            float ratioY2 = (f5 * getRatioY()) / getRatioX();
            RectF rectF2 = this.mFrameRect;
            rectF2.top = ratioY2 + rectF2.top;
        }
        if (!isInsideVertical(this.mFrameRect.top)) {
            float f6 = this.mImageRect.top - this.mFrameRect.top;
            this.mFrameRect.top += f6;
            float ratioX = (f6 * getRatioX()) / getRatioY();
            RectF rectF3 = this.mFrameRect;
            rectF3.left = ratioX + rectF3.left;
        }
    ...
    private void moveHandleRB(float f, float f2) {
        if (this.mCropMode == CropMode.RATIO_FREE) {
            ...
            if (isWidthTooSmall()) {
                ...
                float frameW = getFrameW();
                RectF rectF = this.mFrameRect;
                rectF.right = (f3 - frameW) + rectF.right;
            }
            if (isHeightTooSmall()) {
                float f4 = this.mMinFrameSize;
                float frameH = getFrameH();
                RectF rectF2 = this.mFrameRect;
                rectF2.bottom = (f4 - frameH) + rectF2.bottom;
            }
            checkScaleBounds();
            return;
        }
        ...
        if (!isInsideHorizontal(this.mFrameRect.right)) {
            float f5 = this.mFrameRect.right - this.mImageRect.right;
            this.mFrameRect.right -= f5;
            this.mFrameRect.bottom -= (f5 * getRatioY()) / getRatioX();
        }
        if (!isInsideVertical(this.mFrameRect.bottom)) {
            float f6 = this.mFrameRect.bottom - this.mImageRect.bottom;
    ...
    public Bitmap getCroppedBitmap() {
        ...
        Bitmap createBitmap = Bitmap.createBitmap(bitmap, Math.round(f - (this.mImageRect.left / this.mScale)), Math.round(f2 - (this.mImageRect.top / this.mScale)), Math.round((this.mFrameRect.right / this.mScale) - f), Math.round((this.mFrameRect.bottom / this.mScale) - f2), (Matrix) null, false);
        return this.mCropMode == CropMode.CIRCLE ? getCircularBitmap(createBitmap) : createBitmap;
    }
    ...
    public Bitmap getRectBitmap() {
        ...
        float f = this.mFrameRect.left / this.mScale;
        float f2 = this.mFrameRect.top / this.mScale;
        return Bitmap.createBitmap(bitmap, Math.round(f - (this.mImageRect.left / this.mScale)), Math.round(f2 - (this.mImageRect.top / this.mScale)), Math.round((this.mFrameRect.right / this.mScale) - f), Math.round((this.mFrameRect.bottom / this.mScale) - f2), (Matrix) null, false);
    }
