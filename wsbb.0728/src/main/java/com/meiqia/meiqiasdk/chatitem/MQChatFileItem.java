...
public class MQChatFileItem extends MQBaseCustomCompositeView implements View.OnTouchListener {
    ...
    private void displayFileInfo() {
        ...
        if (MQUtils.isFileExist(MQUtils.getFileMessageFilePath(this.mFileMessage))) {
            ...
            if (parseTimeToLong <= 0) {
                ...
                string = getContext().getString(R.string.mq_expire_after, new Object[]{format});
    ...
    private void openFile() {
        ...
        if (Build.VERSION.SDK_INT >= 29) {
            ...
            fromFile = FileProvider.getUriForFile(context, getContext().getPackageName() + ".fileProvider", file);
        } else {
            fromFile = Uri.fromFile(new File(MQUtils.getFileMessageFilePath(this.mFileMessage)));
        }
        Intent intent = new Intent("android.intent.action.VIEW");
        intent.setDataAndType(fromFile, ShareContentType.FILE);
