...
public class DeliveryHeader extends InternalAbstract implements RefreshHeader {
    ...
    public void dispatchDraw(Canvas canvas) {
        ...
        int height2 = this.mBoxDrawable.getBounds().height();
        int i3 = width / 2;
        this.mBoxDrawable.getBounds().offsetTo(i3 - (this.mBoxDrawable.getBounds().width() / 2), ((((this.mHeaderHeight / 2) - height2) + i2) - Math.min((this.mHeaderHeight / 2) - this.mBoxDrawable.getBounds().height(), DensityUtil.dp2px(this.mAppreciation * 100.0f))) - (this.mBoxDrawable.getBounds().height() / 4));
        this.mBoxDrawable.draw(canvas);
        if (this.mState == RefreshState.Refreshing || this.mState == RefreshState.RefreshFinish) {
            Rect bounds = this.mUmbrellaDrawable.getBounds();
            int i4 = this.mHeaderHeight;
            this.mUmbrellaDrawable.getBounds().offsetTo(i3 - (bounds.width() / 2), ((i2 - i4) + Math.min(this.mHeaderHeight, DensityUtil.dp2px(this.mAppreciation * 100.0f))) - bounds.height());
    ...
    public void onMoving(boolean z, float f, int i, int i2, int i3) {
        if (this.mState != RefreshState.Refreshing) {
            this.mBoxDrawable.setAlpha((int) ((1.0f - Math.max(CropImageView.DEFAULT_ASPECT_RATIO, f - 1.0f)) * 255.0f));
        }
    }
    ...
    public void onStateChanged(@NonNull RefreshLayout refreshLayout, @NonNull RefreshState refreshState, @NonNull RefreshState refreshState2) {
        ...
        if (refreshState2 == RefreshState.None) {
            this.mAppreciation = CropImageView.DEFAULT_ASPECT_RATIO;
    ...
    public void setPrimaryColors(@ColorInt int... iArr) {
        if (iArr.length > 0) {
            ...
            if (iArr.length > 1 && (this.mCloudDrawable instanceof PathsDrawable)) {
                ((PathsDrawable) this.mCloudDrawable).parserColors(iArr[1]);
            }
        }
    }
}
