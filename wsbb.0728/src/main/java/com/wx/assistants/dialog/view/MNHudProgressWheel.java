...
public class MNHudProgressWheel extends View {
    private static final String TAG = "MNHudProgressWheel";
    private int barColor = -1442840576;
    private float barExtraLength = CropImageView.DEFAULT_ASPECT_RATIO;
    private boolean barGrowingFromFront = true;
    private final int barLength = 16;
    private final int barMaxLength = SubsamplingScaleImageView.ORIENTATION_270;
    ...
    private float mProgress = CropImageView.DEFAULT_ASPECT_RATIO;
    private float mTargetProgress = CropImageView.DEFAULT_ASPECT_RATIO;
    ...
    public MNHudProgressWheel(Context context, AttributeSet attributeSet) {
        ...
        parseAttributes(context.obtainStyledAttributes(attributeSet, R.styleable.MNHudProgressWheel));
        setAnimationEnabled();
    }
    ...
    private void setAnimationEnabled() {
        this.shouldAnimate = (Build.VERSION.SDK_INT >= 17 ? Settings.Global.getFloat(getContext().getContentResolver(), "animator_duration_scale", 1.0f) : Settings.System.getFloat(getContext().getContentResolver(), "animator_duration_scale", 1.0f)) != CropImageView.DEFAULT_ASPECT_RATIO;
    }
    ...
    private void setupBounds(int i, int i2) {
        ...
        if (!this.fillRadius) {
            ...
            int i4 = paddingLeft + ((i3 - min) / 2);
            int i5 = paddingTop + ((((i2 - paddingTop) - paddingBottom) - min) / 2);
        ...
        if (this.shouldAnimate) {
            ...
            if (this.isSpinning) {
                ...
                if (!this.linearProgress) {
                    f = ((float) (1.0d - Math.pow((double) (1.0f - (this.mProgress / 360.0f)), (double) 4.0f))) * 360.0f;
                    f7 = ((float) (1.0d - Math.pow((double) (1.0f - (this.mProgress / 360.0f)), (double) 2.0f))) * 360.0f;
                } else {
                    f = 0.0f;
                }
                canvas.drawArc(this.circleBounds, f - 90.0f, isInEditMode() ? 360.0f : f7, false, this.barPaint);
            }
            if (z) {
                invalidate();
            }
    ...
    public void resetCount() {
        ...
        this.mTargetProgress = CropImageView.DEFAULT_ASPECT_RATIO;
        invalidate();
    }
