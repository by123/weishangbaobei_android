        ...
        try {
            new PerformClickUtils2().checkNodeId(this.autoService, img_first_check_layout_id, executeSpeedSetting + CropImageView.DEFAULT_IMAGE_TO_CROP_BOUNDS_ANIM_DURATION, 100, 600, new PerformClickUtils2.OnCheckListener() {
                public void find(int i) {
                    ...
                    int access$000 = GroupSendGroupUtils.this.picCount - 1;
                    while (true) {
                        int i2 = access$000;
        ...
        try {
            if (this.operationParameterModel.getMessageSendType() != 1 || !MyApplication.enforceable) {
                if (this.operationParameterModel.getMessageSendType() == 2 && MyApplication.enforceable && this.circulateInnerSize <= 0) {
                    this.sendGroupNum++;
                    int i = this.startIndexFromUser == 1 ? this.sendGroupNum + 1 + this.jumpNum : this.sendGroupNum + this.startIndexFromUser + this.jumpNum;
                    updateBottom(0);
                    saveRecord(i);
                }
            } else if (this.circulateInnerSize <= 0) {
                this.sendGroupNum++;
                int i2 = this.startIndexFromUser == 1 ? this.sendGroupNum + 1 + this.jumpNum : this.sendGroupNum + this.startIndexFromUser + this.jumpNum;
                updateBottom(0);
                saveRecord(i2);
            }
            updateBottom(0);
            if (this.circulateInnerSize <= 0 && MyApplication.enforceable) {
                ...
                performClickUtils2.checkNilStringTwo(autoService, "请稍候", "正在发送中", BannerConfig.DURATION, 100, 100, r7);
                return;
            }
        } catch (Exception e) {
            e.printStackTrace();
            ...
            public void find(int i) {
                ...
                new PerformClickUtils2().checkNodeIdSyn(GroupSendGroupUtils.this.autoService, BaseServiceUtils.input_edit_text_id, 300, 100, 300, new PerformClickUtils2.OnCheckListener() {
                    public void find(int i) {
                        ...
                        GroupSendGroupUtils.this.saveRecord(GroupSendGroupUtils.this.startIndexFromUser == 1 ? GroupSendGroupUtils.this.sendGroupNum + 1 + GroupSendGroupUtils.this.jumpNum : GroupSendGroupUtils.this.sendGroupNum + GroupSendGroupUtils.this.startIndexFromUser + GroupSendGroupUtils.this.jumpNum);
                        GroupSendGroupUtils.this.updateBottom(0);
                        PerformClickUtils.executedBackHome(GroupSendGroupUtils.this.autoService, 30, new PerformClickUtils.OnBackMainPageListener() {
                            public void find() {
                                GroupSendGroupUtils.this.executeMain();
